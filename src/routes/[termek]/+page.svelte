<script>
	...
	function buy() {
		if (totalDarab < maxAmount) {
			...
			goto('/termekek?Category='.concat($page.url.searchParams.get('Category')));
		} else {
...
<main class="h-screen w-screen overflow-x-hidden overflow-auto bg-white dark:text-white dark:bg-slate-900 pb-32" in:fade={{duration: 180}}>
	...
	<div class="p-8">
		<div class=" bg-gray-200 dark:bg-slate-800 rounded-xl text-center mb-4 p-3">
			...
			<p class=" text-2xl text-center">{termek}</p>
			...
			{/if}
			<p class={maxAmount != 0 ? '' : 'text-red-500'}>Elérhető: {maxAmount} db</p>
			<p>{description}</p>
		</div>
		...
		{#if darab != 0}
			<div in:fly={{y: 100}} class="fixed -bottom-0.5 left-0 h-32 w-full p-2 bg-slate-100 dark:bg-slate-800">
				<div class="flex items-center justify-around drop-shadow-2xl">
					{#key price}
		...
		{:else}
			<div in:fly={{y: 100}} class="fixed bottom-0 left-0 h-32 w-full p-2 bg-slate-100 dark:bg-slate-800 opacity-90">
				<div class="flex items-center justify-evenly drop-shadow-2xl opacity-50">
					...
					{/key}
					<div class="inline-flex bg-gray-300 dark:bg-slate-700 p-1 rounded-3xl">
						<div disabled class="mx-2 rounded-full"><button class=" p-1 rounded-full dark:text-white text-cyan-500"><i class="fa-solid fa-minus"></i></button></div>
						<div in:fade="{{duration: 50}}" class="mx-1 text-center p-1 rounded-full">0</div>
						<div disabled class="mx-2 rounded-full"><button class=" p-1 rounded-full dark:text-white text-cyan-500"><i class="fa-solid fa-plus"></i></button></div>
					</div>
				</div>
				<div class=" flex w-full mt-2 justify-center">
					<button disabled class=" bg-cyan-600 text-white dark:bg-slate-600 p-2 w-11/12 rounded-2xl text-center disabled:opacity-50"><i class="fa-regular fa-cart-shopping"></i> Kosárhoz adás</button>
					<button disabled class=" bg-cyan-500 hover:bg-cyan-600 dark:hover:bg-slate-500 text-white dark:bg-slate-600 p-2 w-12 rounded-full text-center disabled:opacity-50"><i class="fa-solid fa-star"></i></button>
				</div>
			</div>
		{/if}
	</div>
