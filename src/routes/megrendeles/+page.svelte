<script>
	...
	const bufeClosedValue = $bufeClosed
	$: console.log($bufeClosed);
	console.log(bufeClosedValue);
...
<main class="h-screen w-screen overflow-x-hidden overflow-auto text-slate-900 bg-white dark:text-white dark:bg-slate-900">
	...
	<div class="h-screen w-screen fixed top-0 left-0 bottom-0 right-0 bg-black bg-opacity-60 flex justify-center items-center">
		<div class="flex justify-center items-center flex-col w-3/4 lg:w-2/6 bg-gray-100 dark:bg-slate-800 rounded-lg p-4 text-black dark:text-white drop-shadow-xl select-none">
			...
			<div class="flex w-full my-4 justify-center drop-shadow-lg">
				<button class=" dark:bg-slate-700 dark:hover:bg-slate-600 hover:bg-gray-300 bg-gray-300 p-2 px-3 mr-2 rounded-lg" on:click={() => confirmShow = false}>Mégsem</button>
	...
	<div class="px-8">
		...
		<div class=" bg-slate-100 dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 mb-4">
			{#each Object.keys($cart) as termek, i (i)}
			<div href="#" class=" h-24 p-1 flex flex-row items-center justify-between bg-slate-100 rounded-lg dark:bg-slate-800">
		...
		{#if Object.keys($cart).length != 0}
		<div class="flex flex-col justify-center items-center w-full p-2 bg-slate-100 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg mb-8 ">
				...
				<div class=" flex items-center justify-center text-sm font-medium mb-4">
					...
					{#if $total.ar < 500}
						<div class="flex items-center opacity-70">
							...
							<label for="radio-1" class="ml-2 w-24 text-black dark:text-white">Bankkártya (min. 500Ft)</label>
						</div>
					{:else}
			...
			<div class="text-center">
				...
				<select class="w-full p-2 rounded-lg dark:bg-slate-700 bg-slate-200 focus:outline focus:ring-0 focus:outline-cyan-500  ">
					<option class="w-full border border-gray-200 dark:border-slate-700" selected>Válassz egy átvételi időpontot</option>
			...
			<div class="flex flex-col w-full justify-center items-center mt-5 mb-4">
				{#if $bufeClosed}
					<button disabled class=" bg-cyan-500 text-white dark:bg-slate-600 p-2 rounded-xl w-10/12 mb-0 opacity-60" on:click={() => toast("A büfé jelenleg zárva van", {style: 'color: #fff; background-color: #ef4444;',})}>Rendelés leadása</button><br>

					<p class="text-center mt-0 tex-sm">A büfé jelenleg zárva van.</p>
				{:else}
